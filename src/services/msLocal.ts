...
type EntryInfo = { name: string; folder: boolean; file: boolean };
async function listDir(relPath = ""): Promise<EntryInfo[]> {
...
// ===== XLSX helpers =====
function createInitialWorkbookBuffer(): Buffer {
...
function appendRowToWorkbookBuffer(existing: Buffer, row: any[]): Buffer {
...
export function msLocalRouter(): express.Router {
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const dest = path.join(
          LOCAL_BASE,
          folder,
          req.file.originalname || "arquivo.bin"
        );
        await fs.writeFile(dest, req.file.buffer);
        return res.json({
          ok: true,
          source: "local",
          path: `${folder}/${req.file.originalname}`,
        });
      } catch (e: unknown) {
  ...
  router.post(
    "/excel/append",
    express.json(),
    async (req: Request<{}, any, AppendBody>, res: Response) => {
      try {
        const { title, authors, keywords, summary, conclusion, pdfUrl } =
          req.body || {};
        if (!title)
          return res.status(400).json({ ok: false, error: "title_missing" });
        ...
        let fileBuf: Buffer;
        try {
          fileBuf = await fs.readFile(excelFs);
        } catch {
        ...
        return res.json({
          ok: true,
          source: "local",
          excelPath: "PaperMind/papermind.xlsx",
        });
      } catch (e: unknown) {
        const msg = e instanceof Error ? e.message : "excel_append_failed";
        return res.status(500).json({ ok: false, error: msg });
      }
    }
  );

  // Middleware de erro tipado
  router.use(
